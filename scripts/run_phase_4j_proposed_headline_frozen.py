"""
Official frozen-OSM Phase 4.J Proposed headline run.

Runs only the final Proposed system:
  Proposed: quantile forecast + IRP MILP

This is the frozen 73-day headline counterpart to the frozen B0/B1/B2 baseline
comparison. It does not mutate configs/optimize.yaml.

Expected command from project root:
  python scripts/run_phase_4j_proposed_headline_frozen.py
...
OUT = ROOT / "docs" / "results_frozen" / "phase_4j_proposed_headline_frozen_20260531.json"
...
def apply_frozen_headline_overrides(cfg: dict, forecast_csv: Path) -> dict:
...
def main() -> None:
    ...
    cfg = apply_frozen_headline_overrides(load_config(), forecast_csv)
    daily_log = attach_daily_logger(rh)

    print("=" * 92)
    print("RUN PROPOSED: quantile forecast + IRP MILP, frozen 73-day headline settings")
    print("=" * 92)
    ...
    payload = {
        "purpose": "Official frozen-OSM Proposed 73-day headline run for final thesis comparison.",
        "locked_config": {
    ...
    print("\n" + "=" * 92)
    print("PROPOSED FROZEN HEADLINE COMPLETE")
    print("=" * 92)
    print(
